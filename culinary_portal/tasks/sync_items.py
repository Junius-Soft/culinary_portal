...
def run_item_sync(woocommerce_product_name: str):
	"""
	Attempt to sync/create the Item for a given Portal product name.
	If the upstream Portal_fusion app is installed, delegate to it.
	Otherwise, log and safely skip so Sales Order sync can proceed.
	"""
	try:
		from culinary_portal.tasks.sync_items import run_item_sync as upstream_run_item_sync
	except Exception:
		frappe.logger().info(
